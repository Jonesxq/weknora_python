...
dispatcher 使用 `FOR UPDATE SKIP LOCKED` 分页领取；每个事件只有在 Celery 发送成功后才单独写 `sent_at`，不等待整批统一确认。
...
接收知识条目 ID、渲染后的文本和 Pydantic 输出类型，返回已经验证的结构化结果。候选抽取和摘要调用必须携带 `knowledge_id`，阶段二 fake 也按该 ID 索引预设结果，避免同标题知识条目发生响应碰撞；页面合并仍按 slug 索引。fake 可配置抛出瞬时或永久错误。
...
`GRAPH_WIKI_FAKE_DATA_FILE` 指向 JSON fixture。API、dispatcher 和 Worker 进程读取同一个只读 fixture，避免依赖进程内共享状态。fixture 包含 KB 配置、知识条目、chunks 及模型预设输出。因为 fake 模型端口只接收 `knowledge_id`，该 ID 在整个 fixture 中必须全局唯一。输入拒绝未知字段、重复身份、不规范的 merge slug，以及未归属于已声明知识库的知识条目。
...
Outbox dispatcher 每次处理有限行数，并要求本批行共享同一个非空 claim token。它按顺序逐事件发送并立即单独确认：普通发送失败只释放该事件并继续后续事件，避免坏事件阻塞同批尾部；确认失败时不释放已发送事件，等待 stale claim 重投。取消或进程级 `BaseException` 会释放当前和未处理尾部，已确认前缀保持不动。release 清理失败不能覆盖原始异常，日志只包含事件 ID 和错误类型，不记录 claim token 或 payload。多个 dispatcher 通过数据库行锁安全并行，超过 60 秒仍未发送的 claim 可以被其他 dispatcher 回收。

Celery 发布通过线程执行阻塞的 `apply_async()`。取消等待协程后，线程中的发送仍可能完成，所以 Outbox 到 Worker 是至少一次投递，依赖任务与仓储幂等；稳定 `task_id` 用于追踪，不视为 broker 去重承诺。dispatcher 循环在空轮也休眠，连续异常按轮次指数退避并封顶 60 秒，成功轮会重置退避。

阶段二的 claim 与 `available_at` 使用进程侧 `datetime.now(UTC)` 生成；测试只验证先后顺序和允许范围，不对数据库时钟与应用时钟做零误差相等断言。统一改用数据库时钟可在后续阶段单独迁移。
...
- Redis 连接和 token 确认使用有限操作超时，默认 5 秒且必须小于锁 TTL。缓存的锁管理器按租约或状态查询创建独立 Redis 客户端，避免跨事件循环复用连接。
- Worker 提交结果事务前必须调用 `await lease.assert_owned()` 同步比较 token 并续期，不能只读取 `lost` 状态。续期返回假、Redis 异常或操作超时都会设置 `lost=True` 并抛出 `LockOwnershipLost`。
...
领取 pending-op 的同一事务按 `claim_token` 写入一条 `available_at = claimed_at + claim_timeout` 的恢复 Outbox。正常结果提交或完整失败释放会取消该事件；Worker 在 claim 后异常退出时，恢复事件保证在 claim 过期后重新触发。stale claim 被重新领取时取消旧 token 的恢复事件并为新 token 建立新的恢复事件。
...
- slug 必须规范化为小写、总长度不超过 255，并限制为非空安全字符分段；连续斜杠和尾随斜杠无效。
- aliases 清洗空值并保持稳定去重。
...
1. 校验 completed 与 failed 操作互不重复、互不重叠，并且全部属于同一非空 `claim_token`。
2. 再次验证所有待提交来源仍有效。
3. Reduce 前读取冻结的 `page_id + version + ReducedPage` 快照；提交时锁定活跃页面并执行乐观 CAS，期望不存在的页面也必须仍不存在。
4. 创建或更新页面并设置为 `draft`。
5. 原子替换正文对应的链接边。
6. 将本批页面统一设置为 `published`。
7. 每个批次追加一条带稳定 `operation_id` 的日志。
8. 删除成功 pending-op 并释放对应 finalization marker；失败 pending-op 在同一事务中增加 `fail_count` 并清除 claim。
9. 仍有 pending 时在同一事务写入 5 秒后的 follow-up Outbox。

任何一步失败都回滚整个结果事务，不会暴露半成品页面。claim 丢失抛出 `ClaimLost`，页面快照失效抛出 `PageConflict`，调用不变量错误抛出 `InvariantError`；这些错误不会编译出 `claim_token IS NULL` 的更新条件。由于成功/失败 pending-op 处理、日志、页面、follow-up Outbox 和 finalization 释放在同一事务中，重复投递不会重复增加页面版本或日志，也不会在失败释放后丢失后续调度。
...
- Celery 同步任务只复用进程级后台事件循环，fork 后按新 PID 重建并在进程退出时关闭。每个批次都在当前循环内创建新的 runtime，Worker 完成、异常或取消后均在同一循环执行 `await runtime.aclose()`；AsyncEngine、session factory、store 和 Worker 不跨批、loop 或 PID 缓存。普通关闭错误不能覆盖已有 Worker 异常或取消。
- Worker 获得锁但暂时没有可领取行、同时仍存在 pending-op 时按 busy 重试；`PageConflict` 会原子释放整批 claim 并安排 follow-up，重新读取页面快照后再 Reduce。
- Redis 锁续期失败时停止提交结果；Worker 在提交前通过 `await lease.assert_owned()` 再次同步确认，返回假、异常或超时均不允许写入。
