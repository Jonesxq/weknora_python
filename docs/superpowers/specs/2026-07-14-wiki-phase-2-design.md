...
`GRAPH_WIKI_FAKE_DATA_FILE` 指向 JSON fixture。API、dispatcher 和 Worker 进程读取同一个只读 fixture，避免依赖进程内共享状态。fixture 包含 KB 配置、知识条目、chunks 及模型预设输出。输入拒绝未知字段、重复的租户/知识库/知识条目标识，以及未归属于已声明知识库的知识条目。
