...
def detect_excel_format(content: bytes) -> str | None:
    ...
    # Pandas 3 不再接受裸 bytes，这里统一包装成文件对象，同时兼容 Pandas 2。
    ext = inspect_excel_format(content_or_path=BytesIO(content))
...
def find_soffice() -> Optional[str]:
    ...
    # shutil.which 同时支持 Windows 和类 Unix 系统，不依赖外部 which 命令。
