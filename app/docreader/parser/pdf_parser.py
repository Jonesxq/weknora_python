...
* Pages are classified independently so hybrid PDFs (some native, some scanned)
  ...
  Python RAG ingestion layer can perform OCR/VLM on them.

No external parsing service is required: the builtin engine uses pypdfium2.
OCR is intentionally outside this module.
"""
...
from app.docreader.parser.base_parser import BaseParser
from app.docreader.parser.concurrency import parser_worker_limit
...
# the RAG ingestion layer can OCR/caption them. Logos, icons,
...
def _effective_scale(page, scale: float, max_edge: int) -> float:
    ...
    DPI scale produces 100+ MP JPEGs that consume excessive memory and are far
    higher resolution than OCR needs.
    """
    if max_edge <= 0:
        return scale
    width, height = page.get_size()
...
def _select_mp_context():
    ...
    avoiding fork hazards when parsing is called from a multithreaded app.
...
class PDFScannedParser(BaseParser):
    ...
    Used as a robust last-resort fallback and for image-only PDFs. The Python
    RAG ingestion layer performs OCR on the extracted page images.
    """
...
class PDFParser(BaseParser):
    ...
    For each page:
      ...
      * scanned page      -> render to JPEG, tag ``image_source_type=scanned_pdf``
                             so the RAG ingestion layer can OCR it
